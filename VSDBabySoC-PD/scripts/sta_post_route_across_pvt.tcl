...
set STA_OUTPUT_DIR \
    "$ORFS_FLOW_ROOT/sta_output/post_route"

set ODB_FILE \
    "$RESULTS_DIR/6_final.odb"

set SDC_FILE \
    "$RESULTS_DIR/6_final.sdc"

set SPEF_FILE \
    "$RESULTS_DIR/6_final.spef"
...
if {![file exists $ODB_FILE]} {
    puts "ERROR: Synthesis database not found:"
    puts "       $ODB_FILE"
    exit 1
}

if {![file exists $SDC_FILE]} {
    puts "ERROR: Synthesis SDC not found:"
    puts "       $SDC_FILE"
    exit 1
}
...
if {![file exists $SPEF_FILE]} {
    puts "ERROR: SPEF file not found:"
    puts "       $SPEF_FILE"
    exit 1
}
...
puts $WNS_SUMMARY "Library WNS"

puts $TNS_SUMMARY "Library TNS"
...
foreach LIB_NAME $PVT_LIBS {
    ...
    puts $FP "puts \"Reading synthesis database...\""
    ...
    puts $FP "puts \"Reading synthesis SDC...\""
    puts $FP "read_sdc {$SDC_FILE}"

    puts $FP "puts \"Reading parasitics (SPEF)...\"" 
    puts $FP "read_spef {$SPEF_FILE}"
    ...
    puts $FP \
        "report_design_area > {$CORNER_DIR/design_area.txt}"
    ...
    # Min + max
    puts $FP \
        "report_checks -path_delay min_max -fields {cap slew input_pins fanout} -digits 4 > {$CORNER_DIR/report_checks_min_max.txt}"


    # Setup / maximum delay
    puts $FP \
        "report_checks -path_delay max -fields {cap slew input_pins fanout} -digits 4 > {$CORNER_DIR/report_checks_max.txt}"


    # Hold / minimum delay
    puts $FP \
        "report_checks -path_delay min -fields {cap slew input_pins fanout} -digits 4 > {$CORNER_DIR/report_checks_min.txt}"
    ...
    puts $FP \
        "report_worst_slack -max -digits 4 > {$CORNER_DIR/worst_slack_max.txt}"

    puts $FP \
        "report_worst_slack -min -digits 4 > {$CORNER_DIR/worst_slack_min.txt}"
    ...
    puts $FP \
        "report_wns -digits 4 > {$CORNER_DIR/wns.txt}"

    puts $FP \
        "report_tns -digits 4 > {$CORNER_DIR/tns.txt}"
    ...
    puts $FP \
        "report_checks -path_delay min_max -group_count 10 -digits 4 > {$CORNER_DIR/timing_report.txt}"
    ...
    puts $FP \
        "puts \"STA completed for $CORNER_NAME\""
    ...
    # ------------------------------------------------
    # Extract worst max slack
    # ------------------------------------------------

    set MAX_FILE \
        "$CORNER_DIR/worst_slack_max.txt"
    ...
    # ------------------------------------------------
    # Extract worst min slack
    # ------------------------------------------------

    set MIN_FILE \
        "$CORNER_DIR/worst_slack_min.txt"
...
puts ""
puts "================================================"
puts "Post-route PVT STA sweep completed."
puts "================================================"
puts ""
puts "Route database:"
puts "  $ODB_FILE"
